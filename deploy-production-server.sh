...
# Server IP: 178.128.52.192 | Domain: kaksedthan.com & www.kaksedthan.com
# ==============================================================================
...
echo -e "${BOLD}${CYAN}🚀 Initializing KAKSEDTHAN DigitalOcean Production Setup...${RESET}"
...
# 3. Setup Nginx Reverse Proxy for Domain
echo -e "${BOLD}[3/6] Configuring Nginx reverse proxy for kaksedthan.com...${RESET}"
sudo tee /etc/nginx/sites-available/kaksedthan > /dev/null << 'EOF'
server {
    listen 80;
    listen [::]:80;
    server_name kaksedthan.com www.kaksedthan.com 178.128.52.192;
...
if certbot --nginx -d kaksedthan.com -d www.kaksedthan.com --non-interactive --agree-tos -m admin@kaksedthan.com --redirect; then
  echo -e "${GREEN}✓ SSL HTTPS Certificate successfully issued & configured!${RESET}"
else
  echo -e "${YELLOW}⚠️ DNS propagation for kaksedthan.com may still be in progress.${RESET}"
  echo -e "${YELLOW}   Run 'sudo certbot --nginx -d kaksedthan.com -d www.kaksedthan.com' manually once GoDaddy DNS updates.${RESET}"
fi

echo -e "\n${BOLD}${GREEN}======================================================================${RESET}"
echo -e "${BOLD}${GREEN} 🎉 KAKSEDTHAN Production Deployment Complete!${RESET}"
echo -e "${BOLD}${GREEN}======================================================================${RESET}"
echo -e "   • Public Domain (HTTPS):  ${CYAN}https://kaksedthan.com${RESET}"

...
export class BreedingRepository {
  ...
  private mapRowToBreedingRecord(row: any): BreedingRecord {
    return {
      ...
      pregnancyCheckDate: row.pregnancy_check_date ? new Date(row.pregnancy_check_date).toISOString() : null,
      expectedCalvingDate: row.expected_calving_date ? new Date(row.expected_calving_date).toISOString() : null,
      actualCalvingDate: row.actual_calving_date ? new Date(row.actual_calving_date).toISOString() : null,
      calfId: row.calf_id || null,
      notes: row.notes || '',
      ...
      breedingMethod: row.breeding_method || 'Cross-Breeding',
      targetBreed: row.target_breed || undefined,
      bullName: row.bull_name || undefined,
      heatDetectionDate: row.heat_detection_date ? new Date(row.heat_detection_date).toISOString() : null,
      checkupDate: row.checkup_date ? new Date(row.checkup_date).toISOString() : null,
      expectedBirthdate: row.expected_birthdate ? new Date(row.expected_birthdate).toISOString() : null,
